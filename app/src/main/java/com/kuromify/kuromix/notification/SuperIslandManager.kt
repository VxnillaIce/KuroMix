    ...
    fun showMirrorNotification(context: Context, packageName: String) {
        ...
        try {
            ...
            val notification = NotificationCompat.Builder(context, CHANNEL_ID)
                .setSmallIcon(R.drawable.kuromify_logo_w) // status bar / chip icon = mirrored app's icon
                .setLargeIcon(appIconCompat.toIcon(context))
                .setContentTitle("KuroMix")
                .setContentText("Active: $appLabel")
                // No .setStyle(...) — "Standard" style, still eligible for promotion
                .setOngoing(true)
            ...
            Log.d(TAG, "[KUROMIX_LOG] Live Update notification sent")
        } catch (e: Exception) {
